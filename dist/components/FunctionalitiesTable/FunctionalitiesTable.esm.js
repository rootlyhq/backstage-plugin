...
const FunctionalitiesTable = ({ organizationId, params }) => {
  ...
  const nameColumn = useCallback(
    (rowData) => {
      return /* @__PURE__ */ React.createElement(Tooltip, { title: rowData.attributes.description?.substring(0, 255) || rowData.attributes.name }, /* @__PURE__ */ React.createElement(Link, { target: "blank", href: rootlyClient.getFunctionalityDetailsURL(rowData) }, rowData.attributes.name));
    },
    [rootlyClient]
  );
