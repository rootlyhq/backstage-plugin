...
import { RootlyIncident, RootlyFunctionality, RootlyApi, ROOTLY_ANNOTATION_ORG_ID } from '@rootly/backstage-plugin-common';
...
const getViewIncidentsForFunctionalityLink = (functionality: RootlyFunctionality, rootlyClient: RootlyApi) => {
  return {
...
export const RootlyOverviewFunctionalityCard = () => {
  ...
  const functionality =
    functionalityResponse && functionalityResponse.data && functionalityResponse.data.length > 0 ? functionalityResponse.data[0] : null;
          ...
          <HeaderIconLinkRow
            links={
              !functionalityLoading && functionality
                ? [createIncidentLink, getViewIncidentsForFunctionalityLink(functionality, rootlyClient), viewIncidentsLink]
